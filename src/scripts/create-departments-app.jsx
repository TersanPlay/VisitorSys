// Script para criar os 17 departamentos - versão para integração na aplicação React
import { departmentService } from '../services/departmentService';
import { toast } from 'react-hot-toast';

// Componente React para criar departamentos
export const CreateDepartmentsComponent = () => {
  const createDepartments = async () => {
    ...
    toast.loading(`Iniciando a criação de ${departments.length} departamentos...`);
    ...
    for (let i = 0; i < departments.length; i++) {
      ...
      try {
        toast.loading(`Criando departamento ${i + 1}/${departments.length}: ${dept.name}`);
        await departmentService.createDepartment(dept);
        successCount++;
        toast.success(`Departamento ${dept.name} criado com sucesso!`);
      } catch (error) {
        errorCount++;
        toast.error(`Erro ao criar departamento ${dept.name}: ${error.message}`);
        console.error(`Erro ao criar departamento ${dept.name}:`, error);
      }
    }

    toast.success(`Processo concluído! ${successCount} departamentos criados com sucesso. ${errorCount} erros.`);
  };

  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Criar Departamentos</h2>
      <p className="mb-4">Este processo irá criar 17 departamentos pré-configurados no sistema.</p>
      <button
        onClick={createDepartments}
        className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md"
      >
        Iniciar Criação de Departamentos
      </button>
    </div>
  );
};

// Função para uso direto (sem componente React)
export const createDepartmentsDirectly = async () => {
  // Lista de departamentos a serem criados
  const departments = [
    // ... (mesma lista de departamentos do componente acima)
  ];
