...
// Importar o serviço de departamentos (já disponível globalmente na aplicação)
const createDepartments = async () => {
  // Verificar se o serviço está disponível
  if (!window.departmentService) {
  ...
  for (let i = 0; i < departments.length; i++) {
    ...
    try {
      ...
      await window.departmentService.createDepartment(dept);
