// Script simples para criar os 17 departamentos - versão para console do navegador
// Para usar: copie e cole este script no console do navegador quando estiver na página do sistema
...
async function createDepartments() {
  ...
  console.log(`Departamentos existentes: ${existingDepartments.length}`);
  ...
  for (let i = 0; i < departments.length; i++) {
    ...
    try {
      ...
      // Verificar se já existe um departamento com o mesmo nome ou código
      const existingDept = existingDepartments.find(d => d.name === dept.name || d.code === dept.code);
      if (existingDept) {
        console.warn(`⚠️ Departamento ${dept.name} (${dept.code}) já existe. Pulando...`);
        continue;
      }
  ...
  console.log(`Processo concluído! ${createdCount} departamentos criados com sucesso.`);
  console.log('Recarregue a página para ver os novos departamentos.');
}

// Executar a função
console.log('Para criar os departamentos, execute a função createDepartments():');
